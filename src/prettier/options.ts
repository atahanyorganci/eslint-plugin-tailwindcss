...
import type { Customizations } from "./types.js";
import "./index.js";
...
export function getCustomizations(
	...
	const staticAttrs = new Set<string>(defaults.staticAttrs);
	const dynamicAttrs = new Set<string>(defaults.dynamicAttrs);
	const functions = new Set<string>(defaults.functions);

	// Create a list of "static" attributes
	for (const attr of options.tailwindAttributes ?? []) {
	...
	for (const fn of options.tailwindFunctions ?? []) {
		functions.add(fn);
	}
