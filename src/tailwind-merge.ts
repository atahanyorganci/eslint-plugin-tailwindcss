...
export const MODIFIER_SEPARATOR_LENGTH = MODIFIER_SEPARATOR.length;
...
/**
 * In Tailwind CSS v4 important modifier is at the end of the base class name.
 *
 * @param baseClassName - The base class name to strip the important modifier from.
 * @returns The base class name without the important modifier.
 * @see https://tailwindcss.com/docs/styling-with-utility-classes#using-the-important-modifier
 */
function stripImportantModifier(baseClassName: string): [string, boolean] {
	if (baseClassName.endsWith(IMPORTANT_MODIFIER)) {
		return [baseClassName.substring(0, baseClassName.length - 1), true];
	}
	return [baseClassName, false];
}
...
export type ClassObject = Record<string, readonly ClassDefinition[]>;
...
export interface Config {
	 ...
	 */
	conflictingClassGroups: Partial<Record<string, readonly string[]>>;
	/**
	 ...
	 */
	conflictingClassGroupModifiers: Partial<Record<string, readonly string[]>>;
	/**
...
function processClassesRecursively(classGroup: ClassGroup, classPartObject: ClassPartObject, classGroupId: string,	theme: ThemeObject) {
...
const arbitraryVariableRegex = /^\((?:(\w[\w-]*):)?(.+)\)$/;
const fractionRegex = /^\d+\/\d+$/;
const tshirtUnitRegex = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/;
const lengthUnitRegex = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq([whib]|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/;
...
const isInteger = (value: string) => !!value && Number.isInteger(Number(value));

const isPercent = (value: string) => value.endsWith("%") && isNumber(value.slice(0, -1));
...
function isArbitraryValue(value: string) {
	return arbitraryValueRegex.test(value);
}

function isAnyNonArbitrary(value: string) {
	return !isArbitraryValue(value) && !isArbitraryVariable(value);
}

const isArbitrarySize = (value: string) => getIsArbitraryValue(value, isLabelSize, isNever);

function isArbitraryLength(value: string) {
	return getIsArbitraryValue(value, isLabelLength, isLengthOnly);
}

function isArbitraryNumber(value: string) {
	return getIsArbitraryValue(value, isLabelNumber, isNumber);
}

function isArbitraryPosition(value: string) {
	return getIsArbitraryValue(value, isLabelPosition, isNever);
}

const isArbitraryImage = (value: string) => getIsArbitraryValue(value, isLabelImage, isImage);

function isArbitraryShadow(value: string) {
	return getIsArbitraryValue(value, isLabelShadow, isShadow);
}

function isArbitraryVariable(value: string) {
	return arbitraryVariableRegex.test(value);
}

function isArbitraryVariableLength(value: string) {
	return getIsArbitraryVariable(value, isLabelLength);
}

function isArbitraryVariableFamilyName(value: string) {
	return getIsArbitraryVariable(value, isLabelFamilyName);
}

function isArbitraryVariablePosition(value: string) {
	return getIsArbitraryVariable(value, isLabelPosition);
}

const isArbitraryVariableSize = (value: string) => getIsArbitraryVariable(value, isLabelSize);

function isArbitraryVariableImage(value: string) {
	return getIsArbitraryVariable(value, isLabelImage);
}

function isArbitraryVariableShadow(value: string) {
	return getIsArbitraryVariable(value, isLabelShadow, true);
}

function getIsArbitraryValue(value: string,	testLabel: (label: string) => boolean,	testValue: (value: string) => boolean) {
...
function getIsArbitraryVariable(value: string,	testLabel: (label: string) => boolean,	shouldMatchNoLabel = false) {
