...
function createParser(
	...
	const customizationDefaults: Customizations = {
	...
	return {
		...
		preprocess(code: string, options: ParserOptions) {
			const original = base.originalParser(parserFormat, options);
		...
		async parse(text: string, options: ParserOptions) {
			...
			const ast = await original.parse(text, options, options);
...
function transformDynamicAngularAttribute(attr: any, env: TransformerEnv) {
	const directiveAst = tryParseAngularAttribute(attr.value, env);
...
function transformDynamicJsAttribute(attr: any, env: TransformerEnv) {
	const { functions } = env.customizations;

	const ast = recast.parse(`let __prettier_temp__ = ${attr.value}`, {
	...
	astTypes.visit(ast, {
		visitLiteral(path) {
			const entries = Array.from(ancestors(path));
			const concat = entries.find((entry) => {
				return (
					entry.parent
					&& entry.parent.value
			...
			if (isStringLiteral(path.node)) {
				const sorted = sortStringLiteral(path.node, {
					env,
					collapseWhitespace: {
				...
				if (sorted) {
					...
					const value = jsesc(path.node.value, {
		...
		visitTemplateLiteral(path) {
			...
			const sorted = sortTemplateLiteral(path.node, {
				env,
				collapseWhitespace: {
					start: concat?.name !== "right",
					end: concat?.name !== "left",
		...
		visitTaggedTemplateExpression(path) {
			...
			if (isSortableTemplateExpression(path.node, functions)) {
				const sorted = sortTemplateLiteral(path.node.quasi, {
					env,
					collapseWhitespace: {
						start: concat?.name !== "right",
						end: concat?.name !== "left",
...
function transformHtml(ast: any, { env, changes }: TransformerContext) {
	const { staticAttrs, dynamicAttrs } = env.customizations;
...
function transformGlimmer(ast: any, { env }: TransformerContext) {
	const { staticAttrs } = env.customizations;
	...
	function sortAttribute(attr: AttrSingleQuoted | AttrDoubleQuoted) {
		for (let i = 0; i < attr.value.length; i++) {
			...
			if (node.type === "TextNode") {
				const after = sortClasses(node.value, {
					env,
			...
			// @ts-ignore: `LiquidDrop` is for older versions of the liquid plugin (1.2.x)
				...
				visit(node.markup.expression, {
					String(node: any) {
						...
						const after = sortClasses(node.value, { env });
...
function sortStringLiteral(
	...
	const result = sortClasses(node.value, {
		env,
		removeDuplicates,
		collapseWhitespace,
	});
	const didChange = result !== node.value;
	node.value = result;
	...
	const escaped = wasEscaped ? result.replace(/\\/g, "\\\\") : result;
...
function sortTemplateLiteral(
	...
	for (let i = 0; i < node.quasis.length; i++) {
		...
		const same = quasi.value.raw === quasi.value.cooked;
		const originalRaw = quasi.value.raw;
		const originalCooked = quasi.value.cooked;
...
function transformJavaScript(
	...
	const { staticAttrs, functions } = env.customizations;
...
function transformCss(ast: any, { env }: TransformerContext) {
	ast.walk((node: any) => {
		if (node.type === "css-atrule" && node.name === "apply") {
			const isImportant = /\s+(?:!important|#\{(['"]*)!important\1\})\s*$/.test(
				node.params,
			);
	...
	while (nodesToVisit.length > 0) {
		...
		switch (currentNode.type) {
			...
			case "MarkoTagBody":
				...
				break;
			case "MarkoAttribute":
				if (!staticAttrs.has(currentNode.name))
					break;
...
function transformTwig(ast: any, { env, changes }: TransformerContext) {
	...
	for (const child of ast.expressions ?? []) {
...
function transformSvelte(ast: any, { env, changes }: TransformerContext) {
	...
	for (const attr of ast.attributes ?? []) {
		...
		for (let i = 0; i < attr.value.length; i++) {
			...
			if (value.type === "Text") {
				const same = value.raw === value.data;
				...
				visit(value.expression, {
					Literal(node) {
						if (isStringLiteral(node)) {
							...
							const sorted = sortStringLiteral(node, {
								env,
								removeDuplicates: false,
								collapseWhitespace: false,
							});
					...
					TemplateLiteral(node) {
						const before = node.quasis.map((quasi: any) => quasi.value.raw);
						const sorted = sortTemplateLiteral(node, {
							env,
							removeDuplicates: false,
							collapseWhitespace: false,
						});
						...
						if (sorted) {
							for (const [idx, quasi] of node.quasis.entries()) {
								changes.push({
									before: before[idx],
	...
	if (ast.type === "IfBlock") {
		for (const child of ast.else?.children ?? []) {
			transformSvelte(child, { env, changes });
		}
	}

	if (ast.type === "AwaitBlock") {
		const nodes = [ast.pending, ast.then, ast.catch];
...
export const printers: Record<string, Printer> = (function () {
	...
	if (base.printers["svelte-ast"]) {
		function mutateOriginalText(path: any, options: any) {
			...
			if (changes?.length) {
				const finder = lineColumn(options.originalText);
		...
		const original = base.printers["svelte-ast"];
		printers["svelte-ast"] = {
			...original,
